...
/*
 *  Copyright (C) 2000,9 B. Parisse, Institut Fourier, 38402 St Martin d'Heres
 *
...
int main(int ARGC, char *ARGV[]){    
  ...
  if (inemacs){
    ...
    printf("| (c) 2006, 2011 B. Parisse & al (giac), F.Maltey & al (mupacs) |\n");
    putchar(EMACS_DATA_END);
    bool prompt=true;
    for (int k=0;;++k) {
      if (prompt){
  ...
  if ( intexmacs){
    ...
    printf("|     See http://www.gnu.org for license details                    |\n");
    printf("|  May contain BSD licensed software parts (lapack, atlas, tinymt)  |\n");
    printf("| (c) 2003,2011 B. Parisse & al (giac), J. van der Hoeven (TeXmacs) |\n");
    printf("--------------------------------------------------------------------\n");
  ...
  if (ARGC==1){
    ...
    cout << "(c) 2001,2011 B. Parisse & others" << endl;
