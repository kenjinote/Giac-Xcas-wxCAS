...
namespace giac {
  ...
  */
  ...
  static symbolic symb_milieu(const gen & x,const gen & y){
  ...
  static symbolic symb_plotfunc(const gen & a){
  ...
  static define_unary_function_eval (__droite,&giac::_droite,_droite_s);
  define_unary_function_ptr5( at_droite ,alias_at_droite,&__droite,0,true);
  ...
  // circle/sphere->diam
  static gen get_point(const gen & g,int n,GIAC_CONTEXT){
  ...
  static symbolic symb_point(const gen & x,const gen & y){
  ...
  gen _cercle(const gen & args,GIAC_CONTEXT){
	...
	e=gen(v,_SEQ__VECT);
      ...
      gen tmp=_plotimplicit(e,contextptr);
  ...
  static symbolic symb_centre(const gen & args){
  ...
  static symbolic symb_rayon(const gen & args){
  ...
  static symbolic symb_mediatrice(const gen & x,const gen & y){
  ...
  static gen inversion(const gen & centre,const gen & rapport,const gen & arg,GIAC_CONTEXT){
  ...
  */
  ...
  vecteur interdroitecercle(const gen & a,const gen &b,GIAC_CONTEXT){
    ...
    gen I1(ratnormal(pr+d_a*a_pr_perp)),I2(ratnormal(pr-d_a*a_pr_perp));
    if (I1.type==_VECT) 
  ...
  static gen tangent(const gen & args,GIAC_CONTEXT){
	...
	if (v[1].type==_IDNT && find_curve_parametrization(curve,m,v[1],T,tmin,tmax,false,contextptr)){
	  if (re(m,contextptr)==v[1]){
	    direction=subst(direction,v[1],re(M,contextptr),false,contextptr);
  ...
  static define_unary_function_eval (__display,&giac::_couleur,_display_s);
  ...
  */
  ...
  gen _plotseq(const gen & args,const context * contextptr){
	...
	return gensizeerr(gettext("Bad iteration number"));
    }
    else {
      if (l>2){
	if (v[2].type==_INT_)
	...
	return gensizeerr(gettext("Non numeric range value"));
      xmin=x0v[1]._DOUBLE_val;
      xmax=x0v[2]._DOUBLE_val;
      x0=x0v[0];
    }
    if (x0.type!=_DOUBLE_)
      return gensizeerr(gettext("Non numeric initial value"));
    x0d=x0._DOUBLE_val;
