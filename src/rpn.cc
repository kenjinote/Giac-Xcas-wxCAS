...
namespace giac {
  ...
  gen _HPDIFF(const gen & args,GIAC_CONTEXT){
	...
	return _derive(a,contextptr);
      return _HPDIFF(gen(makevecteur(args,x__IDNT_e),_SEQ__VECT),contextptr);
    }
  }
...
#else
      if (vs==2){
	if (v.back()==at_eval) return symb_quote(tmp);
	if (v.back()==at_prod){
	  if (is_integral(tmp))
	    return _ifactor(tmp,context0);
	  return symb_quote(_factor(tmp,context0));
	}
	if (v.back()==at_plus)
	  return symb_quote(_partfrac(tmp,context0)); 
	if (v.back().type==_FUNC)
	  return symb_quote(_convert(gen(makevecteur(tmp,v.back()),_SEQ__VECT),context0));
      }
#endif
      if (v.back().type==_FUNC) // additional check for STO> +/-/etc. if CAS38_DISABLED is set
	return gensizeerr(contextptr);
      if (vs>1)
	return symb_quote(_reorder(makevecteur(tmp,vecteur(v.begin()+1,v.end())),context0));
    }
    tmp=eval(args,1,context0);
    tmp=evalfunc(tmp,contextptr);
    return symb_quote(_recursive_normal(tmp,context0)); // return symb_quote(_partfrac(args,contextptr)); // return symb_quote(recursive_normal(args,1,contextptr)); // symb_quote(simplify(eval(args,1,context0),context0));
  }
  ...
  gen _MAKEMAT(const gen & args,GIAC_CONTEXT){
    ...
    v[1]=_floor(v[1],contextptr);
    v[2]=_floor(v[2],contextptr);
    if (v[1].type!=_INT_ || v[2].type!=_INT_ || v[1].val<1 || v[2].val<1)
      return gensizeerr(contextptr);
    int l=max(v[1].val,1),c=max(v[2].val,1);
