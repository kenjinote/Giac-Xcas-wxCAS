...
namespace giac {
  ...
  symbolic symb_program_sto(const gen & a,const gen & b,const gen & c,const gen & d,bool embedd,GIAC_CONTEXT){
	...
	*logptr(contextptr) << gettext("Warning, defining a derivative function should be done with function_diff or unapply: ") << c << endl;
      if (c.type==_SYMB && c._SYMBptr->sommet!=at_local && c._SYMBptr->sommet!=at_bloc && !lop(c,at_of).empty()){
	...
	*logptr(contextptr) << gettext("Try ") << d.print(contextptr) << ":=unapply(" << c.print(contextptr) << ",";
  ...
  static gen quote_program(const gen & args,GIAC_CONTEXT){
  ...
  static void adjust_sst_at(const gen & name,GIAC_CONTEXT){
  ...
  static void program_leave(const gen & save_debug_info,bool save_sst_mode,debug_struct * dbgptr){
  ...
  gen _program(const gen & args,const gen & name,const context * contextptr){
    ...
    gen res,newres,label;
    // *logptr(contextptr) << & res << endl;
#ifdef RTOS_THREADX
    if ((void *)&res<= (void *)&mainThreadStack[1024]) { 
      return gensizeerr(gettext("Too many recursions"));
    }
#else
    ...
    gen prog=args._VECTptr->back(),save_debug_info((*dbgptr->debug_info_ptr));
    if (vars.type!=_VECT)
    ...
    dbgptr->args_stack.push_back(mergevecteur(vecteur(1,name),*values._VECTptr));
	...
	for (;!ctrl_c && it!=itend;++it){
	  ...
	  if (res.type==_STRNG && res.subtype==-1)
	    break;
	  if (res.type==_SYMB){
	    if (findlabel && res._SYMBptr->sommet==at_label && label==res._SYMBptr->feuille)
	      findlabel=false;
	    if (!findlabel && res._SYMBptr->sommet==at_goto){
	      findlabel=true;
  ...
  gen ifte(const gen & args,bool isifte,const context * contextptr){
    if (args.type!=_VECT || args._VECTptr->size()!=3)
      return gensizeerr(gettext("Ifte must have 3 args"));
	...
	if (isifte)
	  return gensizeerr(gettext("Ifte: Unable to check test")); 
	else
	...
	if (debug_ptr(contextptr)->debug_mode){
      ...
      rt=(clause_fausse.type==_SYMB && clause_fausse._SYMBptr->sommet==at_return);
      if (rt)
	...
	if (debug_ptr(contextptr)->debug_mode){
      ...
      rt=(clause_vraie.type==_SYMB && clause_vraie._SYMBptr->sommet==at_return);
      if (rt)
  ...
  gen _for(const gen & args,const context * contextptr){
    ...
    try {
      ...
      string for_in_s;
      gen index_name;
      if (test.is_symb_of_sommet(at_for) && test._SYMBptr->feuille.type==_VECT && test._SYMBptr->feuille._VECTptr->size()==2){
  ...
  */
  ...
  gen _tilocal(const gen & args,const context * contextptr){
	...
	vars.push_back(g._VECTptr->front());
      if (it->type==_SYMB && it->_SYMBptr->sommet!=at_superieur_strict && it->_SYMBptr->sommet!=at_superieur_egal && it->_SYMBptr->sommet!=at_inferieur_strict && it->_SYMBptr->sommet!=at_inferieur_egal)
  ...
  */
  ...
  const unary_function_ptr * const binary_op_tab(){
