...
namespace xcas {
  ...
  void Flv_Table_Gen_widget_print(Flv_Table_Gen * g,Fl_Printer * p,bool eps){
    ...
    int hp=int((p->page_height()-2*margin)/pixel_scale);
    int wp=int((p->page_width()-2*margin)/pixel_scale);
    g->resize(0,0,wp,hp);
  ...
  void setgraph3dprintstate(Fl_Widget * widget,FILE * stream){
	...
	setgraph3dprintstate(gr,stream);
      if (Graph3d * gr=dynamic_cast<Graph3d * >(wid))
	gr->printing=stream;
    }
  }
  ...
  void widget_ps_print(Fl_Widget * widget,const std::string & fname0,bool eps,int pngpdf,bool preview){
    if (!eps)
      pngpdf=0;
#ifdef Fl_Printer_H
...
#else
    Fl_PostScript_File_Device * ptr=new Fl_PostScript_File_Device();
    if (eps){
      ptr->driver()->pw_=int(widget->w()*pixel_scale);
      ptr->driver()->ph_=int(widget->h()*pixel_scale);
    }
    ...
    if (p){
      ...
      if (Fl_PostScript_File_Device * ps=dynamic_cast<Fl_PostScript_File_Device * >(p)){
#ifdef FL_DEVICE
	setgraph3dprintstate(widget,ps->file());
#else
	setgraph3dprintstate(widget,ps->driver()->file());
#endif
      }
