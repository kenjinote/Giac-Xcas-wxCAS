...
#define S_IWUSR 00200
  ...
  static gen delrowscols(const gen & g,bool isrow,GIAC_CONTEXT){
    ...
    if (!ckmatrix(gm) || !interval.is_symb_of_sommet(at_interval) || (f=interval._SYMBptr->feuille).type!=_VECT || f._VECTptr->size()!=2 || (fa=f._VECTptr->front()).type!=_INT_ || (fb=f._VECTptr->back()).type!=_INT_)
      return gentypeerr();
    int shift = xcas_mode(contextptr)!=0 || abs_calc_mode(contextptr)==38;
  ...
  */
  ...
  static gen rsolve(const gen & f0,const gen &u,const gen & n,vecteur & initcond0,GIAC_CONTEXT){
	...
	vecteur idx,idX;
	for (int i=0;i<uvs;++i){
	  idx.push_back(gen("rsolve_x"+print_INT_(i),contextptr));
	  idX.push_back(gen("rsolve_X"+print_INT_(i),contextptr));
	}
	vecteur vofid;
	for (const_iterateur it=vofb.begin();it!=vofb.end();++it){
	...
	return res;
      ...
      vecteur ids;
      for (int i=m;i<=M;++i){
	ids.push_back(gen("rsolve_x"+print_INT_(i-m),contextptr));
      }
      // replace vofun
      vecteur vofid;
      for (const_iterateur it=vofb.begin();it!=vofb.end();++it){
    ...
    if (uvs==1 && vofa.size()==2 && all_bzero){
      gen idy=identificateur("rsolve_y"),x("rsolve_x",contextptr);
