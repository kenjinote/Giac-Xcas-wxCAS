...
extern "C" void Sleep(unsigned int miliSecond);
  ...
  void calc_mode(int b,GIAC_CONTEXT){
    if (b==38 && strcmp(_autoname_().c_str(),"GA")<0)
  ...
  */
  ...
  unsigned int unicode2utf8(const wchar_t * wline,char * line,unsigned int n){
	...
	continue;	
      ...
      if (ch==0x00b0 || ch==0x2032 || ch==0x2033)
	addspace=false;
      k=0;
...
//      if (!rows) return undef;
	...
	return undef;
      if (index>0)
	g=symbolic(archive_function_tab()[index-1],fe);
      else {
	gen res;
